...
public class Main {
    ...
    private static final Path MACOS_PERMANENT_ENV_PATH = Path.of(System.getProperty("user.home")).toAbsolutePath()
            .normalize()
            .resolve(Path.of("Library", "LaunchAgents", "cacerts_replacement.plist"));

    static {
        final Path newCacerts = Path.of(System.getenv("JAVA_HOME"), "lib", "security", "cacerts")
                .toAbsolutePath().normalize();

        final Path oldCacerts = Path.of(System.getenv("JAVA_HOME"), "jre", "lib", "security", "cacerts")
                .toAbsolutePath().normalize();
    ...
    public static void main(final String[] args) throws Exception {
        ...
        while (phase != Phase.EXIT) {
            ...
            { // try parsing action
                ...
                final ActionKey actionKey = phase.actions.keySet().stream()
                        .filter(actKey -> actKey.key.equals(key) || actKey.shortKey.equals(key)).findAny()
                        .orElse(null);
                if (actionKey != null) {
    ...
    public enum Phase {
        INPUT(Map.of("java_home_cacerts - JAVA_HOME/lib/security/cacerts", Main::readJavaHomeCacerts,
                "apple_keychain - Apple 'KeychainStore' without 'System Roots'", Main::readAppleKeychain,
                "cer_file - .cer file (can parse multiple certificates in one file)", Main::readCerFile)),
    ...
    private static void readJavaHomeCacerts(final Console cli) throws Exception {
        ...
        try (var is = Files.newInputStream(JAVA_HOME_CACERTS)) {
    ...
    private static void writeJavaHomeCacerts(final Console cli) throws Exception {
        {
            final Path originalBackup = JAVA_HOME_CACERTS
                    .resolveSibling(JAVA_HOME_CACERTS.getFileName().toString() + ".original");
        ...
        for (final Certificate cert : certificates) {
            final String alias = cert instanceof final X509Certificate x509Cert
        ...
        try (var os = Files.newOutputStream(JAVA_HOME_CACERTS, StandardOpenOption.TRUNCATE_EXISTING,
    ...
    private static void writeMacosPermanentEnvVar(final Console cli) throws Exception {
        ...
        final Path cacertsPath = MACOS_PERMANENT_ENV_PATH
                .resolveSibling(MACOS_PERMANENT_ENV_PATH.getFileName().toString().split("\\.")[0] + ".cacerts");
        try (var os = Files.newOutputStream(cacertsPath, StandardOpenOption.TRUNCATE_EXISTING,
                StandardOpenOption.CREATE, StandardOpenOption.WRITE)) {
            cacertsStore.store(os, DEFAULT_PASSWORD);
        }
        ...
        if (!Files.exists(MACOS_PERMANENT_ENV_PATH)) {
            ...
            final String pList = """
                    ...
                    """
                    .replace("$$ENV_COMMAND$$", Stream
                            .of(setEnvCommand[0], setEnvCommand[1], setEnvCommand[2], "\"" + setEnvCommand[3] + "\"")
    ...
    private static void readCerFile(final Console cli) throws Exception {
        ...
        while (cerPath == null) {
            System.out.print("Input .cer file path (or exit): ");
            try {
                final String line = cli.readLine().trim();
                ...
                if (Files.isRegularFile(input) && input.toString().endsWith(".cer")) {
                    cerPath = input;
                } else {
                    System.out.println("Not a .cer file");
                }
            } catch (Exception e) {
        ...
        try (var is = Files.newInputStream(cerPath)) {
